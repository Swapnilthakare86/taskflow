 ...
 * Filters out zero-value entries and displays a fallback message when no data is available.
 * 
 */
export default function StatusPieChart({ data = [] }) {
  // Filter out entries with zero values to avoid rendering empty states in the chart
  const valid = data.filter(d => d.value > 0);
  
  // Display empty state message if no valid data is available
  if (!valid.length) {
    return <div className="d-flex align-items-center justify-content-center" style={{height:160,color:'#CBD5E1',fontSize:13}}>No data yet</div>;
  }
